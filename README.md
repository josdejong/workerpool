...
- `Pool.exec(method: Function | string, params: Array | null [, options: Object]) : Promise.<*, Error>`<br>
  ...
  - The following options are available:
    ...
    - `transfer: Object[]`. A list of transferrable objects to send to the worker. Not supported by `process` worker type. See [example](./examples/transferableObjects.js) for usage.
...
Transferrable objects can be sent back to the pool using `Transfer` helper class:
