 ...
 * - TypeScript + WASM build (dist/ts/ compiled from src/ts/)
 * - Advanced Pool with worker choice strategies
 * - Work stealing and task affinity features
 *
 * Prerequisites:
 *   npm run build:ts   # Build TypeScript to dist/ts/
 *
 * Run with:
...
// Import JavaScript build
import * as workerpoolJS from '../src/js/index.js';

// Import TypeScript build from dist (compiled from src/ts)
// Note: Run `npm run build:ts` before running this benchmark
import {
  pool as tsPool,
  advancedPool as tsAdvancedPool,
  cpuIntensivePool as tsCpuIntensivePool,
  ioIntensivePool as tsIoIntensivePool,
  mixedWorkloadPool as tsMixedWorkloadPool,
 ...
 */
function compareResults(jsResult: BenchmarkResult, tsResult: BenchmarkResult): void {
 ...
 */
function compareStrategies(results: BenchmarkResult[]): void {
...
async function main(): Promise<void> {
  ...
  const tsPoolInstance = tsPool({ maxWorkers: 4 });
  const advPool = tsAdvancedPool({
    maxWorkers: 4,
    workerChoiceStrategy: 'least-busy',
    enableWorkStealing: true,
  });
  ...
  const tsExec = await benchmark('TS+WASM Task Execution', async () => {
    await tsPoolInstance.exec(fibonacci, [20]);
  }, 200);
  printResults(tsExec);
  ...
  const tsConcurrent = await benchmark('TS+WASM Concurrent Tasks', async () => {
    const promises: Promise<number>[] = [];
    for (let i = 0; i < 50; i++) {
      promises.push(tsPoolInstance.exec(fibonacci, [15]));
    }
    await Promise.all(promises);
  }, 20);
  ...
  const jsQueue = await benchmark('JS Queue Throughput', async () => {
    const promises: Promise<number>[] = [];
    for (let i = 0; i < 100; i++) {
      promises.push(jsPool.exec((x: number) => x * 2, [i]));
    }
    await Promise.all(promises);
  }, 10);
  ...
  const tsQueue = await benchmark('TS+WASM Queue Throughput', async () => {
    const promises: Promise<number>[] = [];
    for (let i = 0; i < 100; i++) {
      promises.push(tsPoolInstance.exec((x: number) => x * 2, [i]));
    }
    await Promise.all(promises);
  }, 10);
  ...
  const advQueue = await benchmark('AdvancedPool Queue Throughput', async () => {
    const promises: Promise<number>[] = [];
    for (let i = 0; i < 100; i++) {
      promises.push(advPool.exec((x: number) => x * 2, [i]));
    }
    await Promise.all(promises);
  }, 10);
  ...
  await tsPoolInstance.terminate();
  ...
  const poolNoStealing = tsAdvancedPool({
    maxWorkers: 4,
    workerChoiceStrategy: 'round-robin',
    enableWorkStealing: false,
  });

  const poolWithStealing = tsAdvancedPool({
    maxWorkers: 4,
    workerChoiceStrategy: 'round-robin',
    enableWorkStealing: true,
  ...
  const poolNoAffinity = tsAdvancedPool({
    maxWorkers: 4,
    workerChoiceStrategy: 'round-robin',
    enableTaskAffinity: false,
  });

  const poolWithAffinity = tsAdvancedPool({
    maxWorkers: 4,
    workerChoiceStrategy: 'round-robin',
  ...
  const cpuPool = tsCpuIntensivePool({ maxWorkers: 4 });
  const ioPool = tsIoIntensivePool({ maxWorkers: 4 });
  const mixedPool = tsMixedWorkloadPool({ maxWorkers: 4 });
  ...
  const strategyManagerResult = await benchmark('WorkerChoiceStrategyManager (10k selections)', () => {
    const manager = new WorkerChoiceStrategyManager('least-busy');
    // Mock workers
    ...
    for (let i = 0; i < 10000; i++) {
      manager.choose(mockWorkers as any);
      if (i % 100 === 0) {
...
export { benchmark, printResults, compareResults, compareStrategies, BenchmarkResult };
