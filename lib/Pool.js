        ...
        worker.exec(task.method, task.params, task.resolver)
          ...
          .catch(function () {
            ...
            if (worker.terminated) {
              ...
              if(me.minWorkers) me.ensureMinWorkers();
            }
            me._next(); // trigger next task in the queue
          });
      }
...
/**
 * Ensures that a minimum of minWorkers are created.  Used internally, should never be called directly.
 */
Pool.prototype.ensureMinWorkers = function() {
  for(var i=this.workers.length;i<this.minWorkers;i++) this.workers.push(new WorkerHandler(this.script));
};
