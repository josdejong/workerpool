...
function Pool(script, options) {
  ...
  if (options && 'minWorkers' in options) {
    if(options.minWorkers===true) {
      this.minWorkers = Math.max(numCPUs - 1, 1);
    } else {
      if (!isNumber(options.minWorkers) || !isInteger(options.minWorkers) || options.minWorkers < 1) {
        throw new TypeError('Option minWorkers must be a positive integer number');
      }
      this.minWorkers = options.minWorkers;
      this.maxWorkers = Math.max(this.minWorkers, this.maxWorkers);     // in case minWorkers is higher than maxWorkers
    }
    for(var i=0;i<this.minWorkers;i++) this.workers.push(new WorkerHandler(this.script));
  }
}
        ...
        worker.exec(task.method, task.params, task.resolver)
            ...
            .catch(function () {
              ...
              if (worker.terminated) {
				...
				if(me.minWorkers) {
				  for(var i=me.workers.length;i<me.minWorkers;i++) me.workers.push(new WorkerHandler(me.script));
				}
              }
