...
describe("Pool", function () {
  ...
  describe("nodeWorker", function () {
    ...
    it("supports process", function () {
      ...
      return pool.exec(add, [3, 4]).then(function (result) {
    ...
    if (WorkerThreads) {
      ...
      it("supports passing options to threads", function () {
        ...
        var pool = createPool({
          minWorkers: 1,
          workerType: "thread",
          workerThreadOpts: { resourceLimits: { maxYoungGenerationSizeMb } },
        });
        var worker = pool.workers[0].worker;

        assert.strictEqual(worker.isWorkerThread, true);
        assert.strictEqual(
          worker.resourceLimits.maxYoungGenerationSizeMb,
  ...
  it("supports forkOpts parameter to pass options to fork", function () {
    var pool = createPool({
      workerType: "process",
      forkOpts: { env: { TEST_ENV: "env_value" } },
    });
    function getEnv() {
      return process.env.TEST_ENV;
    }

    return pool.exec(getEnv, []).then(function (result) {
    ...
    var pool = createPool({
      ...
      onCreateWorker: (opts) => {
        return {
          ...
          forkOpts: {
            ...opts.forkOpts,
            env: { TEST_ENV: `env_value${counter++}` },
          },
        };
      },
    ...
    return Promise.all([
      ...
      .then(function (result) {
        assert.strictEqual(
          result.length,
          3,
          "The creation hook should be called 3 times",
        );
        assert(
          result.includes("env_value0"),
          "result should include the value with counter = 0",
        );
        assert(
          result.includes("env_value1"),
          "result should include the value with counter = 1",
        );
        assert(
          result.includes("env_value2"),
          "result should include the value with counter = 2",
        );
        return pool.terminate();
      })
      .then(function () {
        assert.strictEqual(
          ...
          3,
          "The termination hook should be called 3 times",
        );
        assert(
          terminatedWorkers.includes("env_value0"),
          "terminatedWorkers should include the value with counter = 0",
        );
        assert(
          terminatedWorkers.includes("env_value1"),
          "terminatedWorkers should include the value with counter = 1",
        );
        assert(
          terminatedWorkers.includes("env_value2"),
          "terminatedWorkers should include the value with counter = 2",
        );
      });
  });
  ...
  it("supports stdout/stderr capture via fork", function (done) {
    var pool = createPool(__dirname + "/workers/console.js", {
      workerType: "process",
      emitStdStreams: true,
    });
  ...
  it("should offload functions to multiple workers", function (done) {
    ...
    Promise.all([pool.exec(add, [3, 4]), pool.exec(add, [2, 3])]).then(
      function (results) {
  ...
  it("should handle errors thrown by a worker", function (done) {
    ...
    pool.exec(test).catch(function (err) {
  ...
  it("should run following tasks if a previous queued task is cancelled", function (done) {
    ...
    var p1 = pool.exec(delayed).then(function (result) {
      assert.strictEqual(result, 1);
      assert.strictEqual(reachedTheEnd, true);
    ...
    var p3 = pool.exec(two).then(function (result) {
    ...
    var promise = pool
      ...
      .catch(function (err) {
        assert.ok(
          err
            .toString()
            .match(/Error: Workerpool Worker terminated Unexpectedly/),
        );
  ...
  it("should clear all workers after tasks are finished", function () {
    ...
    pool.exec(test).then(function (result) {
      assert.strictEqual(result, "ok");
    ...
    return pool.terminate(false, 1000).then(function () {
      assert.strictEqual(pool.workers.length, 0);
    });
  });
  ...
  it("should wait for all workers if pool is terminated before multiple concurrent tasks are finished", function (done) {
    ...
    var promises = [pool.exec(test1), pool.exec(test2), pool.exec(test3)];
    ...
    pool.terminate(false, 2000).then(function () {
      assert.strictEqual(pool.workers.length, 0);
      done();
    });
  });
  ...
  it("should wait for all workers if pool is terminated before tasks are finished, even if a task fails", function (done) {
    ...
    var promises = [pool.exec(test1), pool.exec(test2)];
    Promise.all(promises)
      .then(function (results) {
  ...
  it("should cancel any pending tasks when terminating a pool", function () {
    ...
    return pool.terminate(false).then(function () {
      assert.strictEqual(pool.workers.length, 0);
      assert.strictEqual(pool.tasks.length, 0);
  ...
  it("should throw an error when the tasks queue is full", function () {
    ...
    return Promise.all([task1, task2, task3, task4, task5]).then(function () {
      assert.strictEqual(pool.tasks.length, 0);
      assert.strictEqual(pool.workers.length, 2);
  ...
  describe("abort handler", () => {
    it("should not terminate worker if abort listener is defined dedicated worker with Timeout", function (done) {
    ...
    it("should invoke termination timeout for abort handler if timeout period is reached from task timeout", function (done) {
      var pool = createPool(__dirname + "/workers/cleanup-abort.js", {
        maxWorkers: 1,
